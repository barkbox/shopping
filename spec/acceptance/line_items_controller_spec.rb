...
resource 'LineItem', type: :acceptance do
  ...
  post '/carts/:cart_id/line_items?include=cart' do
    parameter :source_id, scope: [:data, :attributes], required: true
    parameter :source_type, scope: [:data, :attributes], required: true
    parameter :sale_price, scope: [:data, :attributes], required: true
    parameter :list_price, scope: [:data, :attributes], required: true
    ...
    let(:expected){
      ...
      {"data" => {
        ...
        "type"=>"line_items", 
        "links"=>{
          "self"=>"http://example.org/line_items/#{li.id}"}, 
          "attributes"=>{
            "cart_id"=>cart_id, 
            "sale_price"=>"1.0", 
            "list_price"=>"1.0",
       ...
       "included"=>[{
        ...
        "type"=>"carts", 
        "links"=>{
          "self"=>"http://example.org/carts/#{cart.id}"}, 
          "attributes"=>{
          ...
          "related"=>"http://example.org/carts/#{cart_id}/line_items"}}, 
          "cart_purchases"=>{
            "links"=>{
  ...
  get '/carts/:cart_id/line_items', document: false do 
    let(:cart_id) { 9999 }
  ...
  get '/carts/:cart_id/line_items', document: true do
    parameter :id, required: true
    parameter :cart_id, required: true
    ...
    example 'Find line_items with an unowned cart' do
      do_request
      expect(status).to eq(200)
      expect(response_json).to eq(expected)
    end

    example 'Find line_items with an owned cart and logged in owner' do
      cart.update!(user_id: 1)
      log_in_user(cart.user_id)
      do_request
      expect(status).to eq(200)
    ...
    example 'Find line_items with an owned cart and logged in non-owner' do
      expected = {:errors=>
        [{:title=>"Index Forbidden",
          :detail=>"You don't have permission to index this shopping/cart.",
          :code=>"403",
  ...
  post '/carts/:cart_id/line_items' do
    parameter :source_id, scope: [:data, :attributes], required: true
    parameter :source_type, scope: [:data, :attributes], required: true
    parameter :sale_price, scope: [:data, :attributes], required: true
    parameter :list_price, scope: [:data, :attributes], required: true
  ...
  patch '/carts/:cart_id/line_items/:id' do
    parameter :id, 'Line item id', required: true
    parameter :cart_id, required: true
  ...
  delete '/carts/:cart_id/line_items/:id' do
    parameter :line_item_id, 'Line item id', required: true
    parameter :cart_id, required: true
